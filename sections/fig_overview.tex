...
% intent binding
\draw[rel] (in)--(gb1) node[midway,left,font=\tiny]{implemented\_by};
\draw[rel, dashed] (in) to[out=0,in=180] (tk);

% block1 internals
\draw[rel] (gb1)--(c1) node[pos=0.28,above,font=\tiny,xshift=-1.2mm]{executes};
\draw[rel] (gb1)--(c2);
\draw[rel] (c1)--(a1) node[pos=0.35,above,font=\tiny]{produces};
\draw[rel] (c2)--(s1) node[midway,below,font=\tiny]{mutates};

% block2 internals + cross-block
\draw[rel] (gb2)--(c3);
\draw[rel] (c3)--(a2);
\draw[rel] (a1) to[out=-90,in=90] node[pos=0.2,right,font=\tiny]{consumes} (c3);
\draw[rel, dotted] (a2) to[out=200,in=-20] node[midway,right,font=\tiny]{equivalent\_to} (a1);
