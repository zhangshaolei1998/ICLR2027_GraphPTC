...
The agent then produces the next intent and program as $(I_{t+1}, P_{t+1}) = \pi(q, \mathcal{H}_t, F_t)$, where $\pi$ denotes the backbone LLM and $\mathcal{H}_t$ holds the task instruction with the programs the agent itself wrote, so the raw outputs of past blocks stay in the runtime and reach the model only through $F_t$.

\subsection{Graph-Based Execution Modeling}
\label{sec:method-modeling}

This stage turns a time-ordered running record into a typed execution graph $\mathcal{G}=(\mathcal{V},\mathcal{E})$ maintained per task, so that result provenance~\citep{herschel2017provenance}, cross-block consumption, and state evolution obtain one representation.
The node set $\mathcal{V}$ covers program blocks, tool calls, artifacts, state versions, and failures, together with a task node and intent nodes, and the edge set $\mathcal{E}$ carries typed relations for execution, data flow, and state (Appendix Table~\ref{tab:types} lists the full type inventory).

\textbf{Typed Trace Projection.}\quad
The first structural change is that a code execution is modeled by its internal effects and not by its output string.
...
\begin{equation}
\{\, b_t \xrightarrow{\mathrm{executes}} c \mid c \in \mathcal{C}_t \,\}
\ \cup\
\{\, c \xrightarrow{\mathrm{produces}} v \mid v \in \mathcal{A}_t \,\}
\ \cup\
\{\, u_\phi \xrightarrow{\mathrm{fails\_at}} \phi \mid \phi \in \Phi_t \,\},
\label{eq:proj}
\end{equation}
where $\mathcal{C}_t$ is the set of calls issued by $b_t$, $\mathcal{A}_t$ the artifacts they produce, $\Phi_t$ the failures, and $u_\phi$ the failing call when the failure is attributable to one call and the block $b_t$ otherwise, and every call $c$ carries a typed effect $\varepsilon(c)\in\{\mathrm{read},\mathrm{write}\}$ with success and state-change flags.
Because calls inside loops and conditionals all enter the graph, multi-tool execution within one block becomes analyzable.

\textbf{Cross-Block Dependency Linking.}\quad
The second structural change is that history across blocks is a multi-relation graph and not a linear order.
...
\begin{equation}
v \xrightarrow{\mathrm{equivalent\_to}} v' \quad\text{when}\quad \kappa(v)=\kappa(v'),
\label{eq:equiv}
\end{equation}
...
The count is advisory and the agent may override it, since a plan change is only suggested once the count passes a threshold.
...
From the accumulated graph, the intent, and the delta, $f_{\mathrm{sum}}$ extracts the declared action, its receipt, the new and equivalent artifacts, the state changes, the failure context, and the prior attempts on the target, and reduces them under the budget by pruning list items and then fields, so that $|F_t| \le B$ and the long-horizon record stays at runtime.
...
\WHILE{budget remains}
    ...
    \ENDIF
    \STATE register $I$ and snapshot $\mathcal{G}^{-} \leftarrow \mathcal{G}$
    ...
    \STATE $\Delta \leftarrow f_{\mathrm{diff}}(\mathcal{G}^{-}, \mathcal{G})$ over new artifacts $\mathcal{N}$, state effects $\mathcal{S}$, failures, stagnation \COMMENT{Sec.~\ref{sec:method-progress}}
